 ...
 */
async function resolveBlocksMedia(
  strapi: any,
  blocuri: unknown,
): Promise<unknown> {
 ...
 */
async function adoptRequestedChildren(
  strapi: any,
  hostId: string,
  blocuri: unknown,
) {
...
export default factories.createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async options() {
      const [pages, index] = await Promise.all([
        strapi
          ...
          .findMany({ sort: { titlu: "asc" }, limit: -1 }),
        loadPageIndex(strapi),
      ]);
    ...
    async createOne(ctx: Context) {
      ...
      if (duplicate)
        return ctx.badRequest("Există deja o pagină cu acest slug");
      ...
      return {
        data: detailView(created, await loadPageIndex(strapi), blocuri),
      };
    },
    ...
    async updateOne(ctx: Context) {
      ...
      return {
        data: detailView(updated, await loadPageIndex(strapi), blocuri),
      };
    },
    ...
    async byPath(ctx: Context) {
      const requested =
        typeof ctx.query.path === "string" ? ctx.query.path : "";
      ...
      const withArticles = await resolveCategoryBlocks(
        strapi,
        withLinks,
        roleType,
      );
