...
import { createMeetingSchema, createMentorMeetingSchema } from "../validation/meeting";
...
export default factories.createCoreController("api::ong.ong", ({ strapi }) => ({
  ...
  async updateMeeting(ctx: Context) {
    ...
    if (!(await claimMeetingStatus(meeting.documentId, "programata"))) {
      return ctx.badRequest("Întâlnirea a fost modificată între timp. Reîncarcă pagina și încearcă din nou.");
    }
  ...
  async ongsForMentor(ctx: Context) {
    ...
    const ongsById = new Map<
      string,
      { documentId: string; name: string; programs: { documentId: string; name: string }[] }
    >();
    for (const row of rows as any[]) {
      ...
      for (const ong of rowOngs) {
        ...
        for (const program of rowPrograms) {
          if (!existing.programs.some((p) => p.documentId === program.documentId)) {
            existing.programs.push({ documentId: program.documentId, name: program.name });
          }
        }
    ...
    return {
      data: Array.from(ongsById.values()).sort((a, b) => a.name.localeCompare(b.name)),
    };
  },
  ...
  async meetingsForMentor(ctx: Context) {
    ...
    const meetings = await strapi.documents("api::meeting.meeting").findMany({
      ...
      sort: { dataOra: "desc" },
      populate: { ong: true, mentor: true, program: true, activityType: true, report: true },
    });
  ...
  async createMeetingForMentor(ctx: Context) {
    ...
    const ong = await strapi.documents("api::ong.ong").findOne({ documentId: data.ong });
    if (!ong) {
      return ctx.badRequest("Organizația nu există");
    }
    const assignmentRows = await strapi.documents("api::ngo-mentor.ngo-mentor").findMany({
      filters: {
        ong: { documentId: ong.documentId },
  ...
  async completeMeetingForMentor(ctx: Context) {
    ...
    if (meeting.status !== "programata") {
      return ctx.badRequest("Doar întâlnirile programate pot fi marcate ca efectuate");
    }
    const updated = await strapi.db.query("api::meeting.meeting").update({
      where: { documentId: meeting.documentId, status: "programata" },
  ...
  async uploadMeetingReportForMentor(ctx: Context) {
    ...
    if (meeting.status !== "efectuata") {
      return ctx.badRequest("Raportul poate fi adăugat doar pentru întâlniri efectuate");
    }
    const uploaded = await uploadSingleFile(ctx);
    if ("error" in uploaded) {
      return ctx.badRequest(uploaded.error);
 ...
 */
async function uploadSingleFile(ctx: Context): Promise<{ id: number } | { error: string }> {
 ...
 */
async function claimMeetingStatus(documentId: string, expectedStatus: string): Promise<boolean> {
...
function mentorMeetingView(meeting: any) {
  return {
    ...
    subiect: meeting.subiect,
    linkIntalnire: meeting.linkIntalnire ?? null,
    ong: meeting.ong ? { documentId: meeting.ong.documentId, name: meeting.ong.name } : null,
    mentor: meeting.mentor
      ? { documentId: meeting.mentor.documentId, nume: meeting.mentor.nume }
      : null,
    program: meeting.program
    ...
    comentarii: meeting.comentarii ?? null,
    report: meeting.report
      ? { url: meeting.report.url, name: meeting.report.name, ext: meeting.report.ext }
      : null,
  };
}
